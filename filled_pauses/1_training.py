...
train = pd.read_csv("data/filled_pauses.csv").drop(
    columns="wavpath duration annotations fileroot".split()
)
train["wavpath"] = train.apply(
    lambda row: "data/train/"
    + row["file"].replace(".exb.xml", "")
    + f"-avd_{row['start_ms']:0.3f}_{row['end_ms']:0.3f}.wav",
    axis=1,
)
wavs_to_train_on = [str(i) for i in Path("data/train").glob("*.wav")]

train["label"] = train.labels.apply(eval).apply(label_processor)
print("Before selecting train split:", train.shape)
train = train[train.wavpath.isin(wavs_to_train_on)].reset_index(drop=True)
print("After selecting train split:", train.shape)

train["audio"] = train.wavpath.values
# train["segment_path"] = train.wavpath.apply(lambda s: Path(s).with_suffix(".wav").name)
train = train.drop(columns=["file", "start_ms", "end_ms", "segment_path", "labels", "wavpath"])
